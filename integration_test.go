...
func integrationTestHelperOrbslam(t *testing.T, subAlgo viamorbslam3.SubAlgo) {
	...
	for i := 0; i < testhelper.GetNumOrbslamImages(subAlgo)-2; i++ {
		...
		for {
			...
			if strings.Contains(line, "Passed image to SLAM") {
				prevNumFiles = slamTesthelper.CheckDeleteProcessedData(t, slam.Mode(subAlgo), dataDir, prevNumFiles, deleteProcessedData, useLiveData)
				break
			}
	...
	testOrbslamDir(t, dataDir, expectedMapsOnline, 2)
	...
	for _, directoryName := range directories {
		files, err := os.ReadDir(dataDir + "/data/" + directoryName)
		test.That(t, err, test.ShouldBeNil)
		lastFileName := files[len(files)-1].Name()
		test.That(t, os.Remove(dataDir+"/data/"+directoryName+lastFileName), test.ShouldBeNil)
	}
	prevNumFiles--
	...
	testOrbslamInternalState(t, svc, dataDir)
	...
	// Test orbslam directory, should have two configs
	testOrbslamDir(t, dataDir, expectedMapsOffline, 2)
	...
	testOrbslamDir(t, dataDir, expectedMapsApriori, 3)
